# Источники данных для исследования Frontend Baseline 2023–2025

## Браузерные релиз-ноуты

### Chrome / Chromium
- **Chrome Releases Blog:** https://chromereleases.googleblog.com/
  - 2023: https://chromereleases.googleblog.com/2023/
  - 2024: https://chromereleases.googleblog.com/2024/
  - 2025: https://chromereleases.googleblog.com/2025/
- **Chrome Platform Status:** https://chromestatus.com/
- **Chrome Developers Blog:** https://developer.chrome.com/blog
- **New in Chrome:** https://developer.chrome.com/new
- **Release Notes:** https://developer.chrome.com/release-notes

**Период:** Версии 109+ (январь 2023) → текущая версия

### Firefox
- **Firefox Release Notes:** https://www.mozilla.org/en-US/firefox/releases/
- **Firefox Developer Release Notes (MDN):** https://developer.mozilla.org/en-US/docs/Mozilla/Firefox/Releases
- **Mozilla Hacks Blog:** https://hacks.mozilla.org/
- **Firefox Release Calendar:** https://whattrainisitnow.com/calendar/
- **MozillaWiki Releases:** https://wiki.mozilla.org/Releases

**Период:** Версии 109+ (январь 2023) → текущая версия

### Safari / WebKit
- **Safari Release Notes:** https://developer.apple.com/documentation/safari-release-notes
- **WebKit Blog:** https://webkit.org/blog/
- **Safari Technology Preview Release Notes:** https://developer.apple.com/documentation/safari-technology-preview-release-notes
- **WebKit Feature Status:** https://webkit.org/status/

**Период:** Safari 16.3+ (январь 2023) → текущая версия

### Microsoft Edge
- **Edge Release Notes:** https://learn.microsoft.com/en-us/deployedge/microsoft-edge-relnote-stable-channel
- **Edge Developer Blog:** https://blogs.windows.com/msedgedev/
- **Edge Platform Status:** https://developer.microsoft.com/en-us/microsoft-edge/status/

**Период:** Синхронизирован с Chrome (версии 109+ → текущая)

## Baseline и совместимость

### Web Platform Baseline
- **Baseline Dashboard:** https://web.dev/baseline
- **web-features Repository:** https://github.com/web-platform-dx/web-features
- **WebDX Community Group:** https://www.w3.org/community/webdx/

### Browser Compatibility Data
- **MDN Browser Compatibility Data (BCD):** https://github.com/mdn/browser-compat-data
- **Can I Use:** https://caniuse.com/
- **MDN Web Docs:** https://developer.mozilla.org/

## Спецификации

### W3C
- **W3C Technical Reports:** https://www.w3.org/TR/
- **W3C Working Drafts:** https://www.w3.org/groups/wg/
- **W3C GitHub Organizations:** https://github.com/w3c

### WHATWG
- **WHATWG Living Standards:** https://spec.whatwg.org/
- **HTML Living Standard:** https://html.spec.whatwg.org/
- **DOM Living Standard:** https://dom.spec.whatwg.org/
- **Fetch Living Standard:** https://fetch.spec.whatwg.org/

### TC39 (ECMAScript)
- **TC39 Proposals:** https://github.com/tc39/proposals
- **TC39 Finished Proposals:** https://github.com/tc39/proposals/blob/main/finished-proposals.md
- **TC39 Website:** https://tc39.es/

### CSS Working Group (CSSWG)
- **CSSWG Drafts:** https://drafts.csswg.org/
- **CSSWG GitHub:** https://github.com/w3c/csswg-drafts
- **CSS Specifications:** https://www.w3.org/Style/CSS/specs.en.html

### Web Incubator Community Group (WICG)
- **WICG GitHub:** https://github.com/WICG
- **WICG Discourse:** https://discourse.wicg.io/

## Interop

### Interop 2023
- **Interop 2023:** https://wpt.fyi/interop-2023
- **Interop 2023 Dashboard:** https://wpt.fyi/results/?label=master&label=experimental&product=chrome&product=firefox&product=safari&aligned&view=interop&q=label%3Ainterop-2023

### Interop 2024
- **Interop 2024:** https://wpt.fyi/interop-2024
- **Interop 2024 Dashboard:** https://wpt.fyi/results/?label=master&label=experimental&product=chrome&product=firefox&product=safari&aligned&view=interop&q=label%3Ainterop-2024

### Interop 2025
- **Interop 2025:** https://wpt.fyi/interop-2025
- **Interop 2025 Dashboard:** https://wpt.fyi/results/?label=master&label=experimental&product=chrome&product=firefox&product=safari&aligned&view=interop&q=label%3Ainterop-2025

## Технические блоги и статьи

### Официальные блоги разработчиков браузеров
- **web.dev (Google):** https://web.dev/
- **Chrome Developers YouTube:** https://www.youtube.com/@ChromeDevs
- **Mozilla Hacks:** https://hacks.mozilla.org/
- **WebKit Blog:** https://webkit.org/blog/
- **Microsoft Edge Blog:** https://blogs.windows.com/msedgedev/

### Эксперты индустрии
- **Jen Simmons (Apple):** https://jensimmons.com/
- **Una Kravets (Google):** https://una.im/
- **Rachel Andrew (Google):** https://rachelandrew.co.uk/
- **Bramus Van Damme (Google):** https://www.bram.us/
- **Miriam Suzanne (Oddbird):** https://www.miriamsuzanne.com/
- **Adam Argyle (Google):** https://nerdy.dev/

### Community ресурсы
- **CSS-Tricks:** https://css-tricks.com/
- **Smashing Magazine:** https://www.smashingmagazine.com/
- **A List Apart:** https://alistapart.com/
- **Frontend Focus Newsletter:** https://frontendfoc.us/

## Issue Trackers

- **Chromium Bugs:** https://bugs.chromium.org/
- **Mozilla Bugzilla:** https://bugzilla.mozilla.org/
- **WebKit Bugzilla:** https://bugs.webkit.org/

## Социальные сети

### Twitter/X (ключевые аккаунты)
- @ChromiumDev
- @FirefoxDevTools
- @webkit
- @MSEdgeDev
- @argyleink (Adam Argyle)
- @Una (Una Kravets)
- @bramus (Bramus Van Damme)
- @TerribleMia (Miriam Suzanne)

### Mastodon
- https://front-end.social/ (Frontend community)

---

**Дата создания:** 18.11.2025
**Исследование:** `frontend-baseline-2023-2025`
