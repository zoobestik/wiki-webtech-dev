...
description: Комплексный обзор изменений в HTML за 2024 год - Popover API, Declarative Shadow DOM, формы, accessibility и практические рекомендации
outline: deep
lastUpdated: true
---
...
```css
dialog {
    transition: opacity 0.3s, overlay 0.3s;
}
```
...
```html
<button commandfor="target-id" command="action-name">
    Текст кнопки
</button>
...
**Chrome**: Реализация в процессе (Intent to Ship)
**Firefox**: Положительная позиция, реализация ожидается
**Safari**: Положительные сигналы, временная шкала неясна
...
```html
<div role="suggestion">
    Предлагаемое изменение: Замените «там» на «тем»
</div>
```
...
```html
<div role="comment" aria-label="Комментарий Ивана">
    Этот параграф требует доработки.
</div>
```
...
```html
<button aria-label="Закрыть" aria-description="Закрывает диалог без сохранения">
    ×
</button>
```

**Назначение**: Предоставление дополнительного описания помимо `aria-label`
**Отличие от `aria-describedby`**: Непосредственно содержит текст vs. ссылается на другой элемент

#### 2. `aria-braillelabel`

```html
<button aria-label="Информация" aria-braillelabel="Инфо">
    ℹ
</button>
```
...
```html
<div role="region" aria-roledescription="Раздел викторины"
     aria-brailleroledescription="Викт">
...
<div inert>
    ...
    <input type="text"> <!-- Нельзя взаимодействовать -->
</div>
```
...
<div lang="ru" dir="rtl">
    <custom-element>
        #shadow-root
            <p>Текст</p> <!-- lang и dir не наследовались -->
    </custom-element>
</div>
```
...
<div lang="ru" dir="rtl">
    <custom-element>
        #shadow-root
            <p>Текст</p> <!-- lang="ru" и dir="rtl" корректно наследуются -->
    </custom-element>
</div>
```
...
**До клавиатуры**: `100vh` = полная высота экрана
**После клавиатуры** (с `resizes-content`): `100vh` = высота экрана минус клавиатура
...
<div contenteditable="plaintext-only">
    Вводите здесь — всё форматирование rich text отключено
</div>
```
...
**Период**: 12 сентября — 5 ноября 2024
**Респонденты**: 5402
**Охват**: Расширен на веб-платформу в целом (браузерные API, проблемы, accessibility, аналитика, производительность)
...
35. Medium: Animate Details Element: https://medium.com/@jgustavo.wd/solved-how-to-fully-animate-the-details-html-element-with-only-css-no-javascript-b7d32c53a9d7
...
39. WeAreDevelopers Magazine: https://www.wearedevelopers.com/en/magazine/506/the-state-of-html-2024-what-can-we-learn-from-it
