...
Период 2018–2022 характеризуется переходом от дискретных версий HTML (HTML5, HTML 5.1, 5.2, 5.3) к
модели **Living Standard** под управлением WHATWG. Ключевые события:
...
**До мая 2019:** параллельная разработка W3C HTML5.x и WHATWG HTML Living Standard создавала
путаницу и фрагментацию.

**После мая 2019:** WHATWG HTML Living Standard — единственный авторитетный источник HTML
спецификации.
...
**Описание:** Нативный элемент для создания диалоговых окон (модальных и немодальных) без
использования JavaScript библиотек.
...
**Начиная с 2022, поддерживается discrete animation для свойства `display`**, что позволяет
анимировать появление/исчезновение диалога.
...
До марта 2022 элемент `<dialog>` поддерживался только в Chrome/Edge (Chromium). Разработчикам
приходилось использовать:

- Polyfills (например, `dialog-polyfill`)
- Библиотеки (Modal компоненты)

Firefox и Safari **догнали** только в марте 2022, что сделало элемент практически применимым для
production без polyfills.
...
**Описание:** Позволяет браузеру отложить загрузку off-screen изображений и iframe до момента, когда
пользователь прокручивает страницу к ним.
...
- 97.5% lazy-loaded изображений ниже fold загружаются полностью в течение 10ms после появления в
  viewport
...
**Описание:** Позволяет указать браузеру, какую иконку/текст отображать на клавише Enter виртуальной
клавиатуры.
...
Улучшает UX на мобильных устройствах, давая пользователю визуальную подсказку о действии при нажатии
Enter.
...
До `inputmode` разработчикам приходилось использовать `type="number"` для числовых полей, что имело
недостатки:
...
**Описание:** Делает элемент и всех его потомков неинтерактивными: они не могут получить фокус,
недоступны для assistive technologies, игнорируются при поиске по странице.
...
В сочетании с `loading` и `fetchpriority` позволяет тонко контролировать загрузку и декодирование
изображений для оптимизации Core Web Vitals (LCP, CLS).

### Атрибут `fetchpriority` — приоритет загрузки ресурсов

**Применяется к:** `<img>`, `<link>`, `<script>`

**Описание:** Указывает браузеру приоритет загрузки ресурса относительно других ресурсов того же
типа.
...
**Статус в 2018–2022:** Все эти атрибуты уже были baseline к началу периода, но активно
использовались и улучшались.
...
В период 2018–2022 ARIA (Accessible Rich Internet Applications) атрибуты получили значительное
развитие:
...
**Статус в 2018–2022:** Все три технологии **уже были baseline** к началу периода, но активно
развивались.
...
**Практическое влияние:** Позволяет серверный рендеринг Web Components без hydration через
JavaScript.
...
В период 2018–2022 существующие типы `<input>` получили улучшения, но **новых типов не
добавлялось**. Основные улучшения:
...
4. **Safari как сдерживающий фактор:**
    - Многие фичи (`<dialog>`, `loading="lazy"`) достигли baseline только после поддержки в Safari
      (март 2022)
